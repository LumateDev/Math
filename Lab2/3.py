...
def modify_hessian(hessian):
    modified_hessian = hessian
    # Добавление диагонального увеличения к главной диагонали
    for i in range(len(modified_hessian)):
        modified_hessian[i][i] += 1e-5  # Примерный размер увеличения
...
def newton_raphson_method(start_point, ep1=0.1, ep2=0.15, M=10):
    ...
    while True:
        ...
        hes = hessian_matrix()
        modified_hes = modify_hessian(hes)  # Модифицируем гессиан
        inv_modified_hes = inverse(modified_hes)
        d = None
        if inv_modified_hes[0][0] > 0 and modified_hes[0][0] * modified_hes[1][1] - modified_hes[0][1] * modified_hes[1][0] > 0:
            d = [- (inv_modified_hes[0][0] * grad[0] + inv_modified_hes[0][1] * grad[1]),
                 - (inv_modified_hes[1][0] * grad[0] + inv_modified_hes[1][1] * grad[1])]
        else:
            d = [-grad[0], -grad[1]]
...
start_point = (1.5, 1.5)
result = newton_raphson_method(start_point)
img_2d(result)
img_3d(result)